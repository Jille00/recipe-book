/**
 * AI extraction of a recipe from free text. Shared by the "From Text" import
 * (app/api/import-recipe-text) and the link import's fallback for pages
 * without structured recipe data (app/api/import-recipe-url).
 */
...
export const TEXT_EXTRACTION_PROMPT = `Extract the recipe information from this text. The text may be copied from a recipe website, cookbook, or other source.
...
/** Longest text sent to the model; anything beyond is cut off. */
export const MAX_EXTRACTION_TEXT_LENGTH = 30000;

/**
 * Ask the model to extract a recipe from `text`. Errors from the AI SDK are
 * passed through for the caller to map to a response.
 */
export async function extractRecipeFromText(
  text: string
): Promise<ExtractionResponse> {
  // Limit content length to avoid token limits
  const maxLength = MAX_EXTRACTION_TEXT_LENGTH;
  const truncatedContent =
    text.length > maxLength
      ? text.slice(0, maxLength) + "\n\n[Content truncated...]"
      : text;

  // Call the model to extract the recipe
