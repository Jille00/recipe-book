...
export const recipe = pgTable(
  "recipe",
  {
    ...
    // The recipe's public address: /r/{code}/{slug}. Generated by the database
    // (see generate_recipe_code in the migration) so every row, including rows
    // inserted by code that predates this column, always has one.
    //
    // It is unguessable on purpose. A recipe that is not listed publicly is
    // still viewable by anyone holding its link, exactly like an unlisted video,
    // so the code is what keeps those recipes from being enumerated.
    code: text("code")
      .notNull()
      .unique()
      .default(sql`public.generate_recipe_code()`),
